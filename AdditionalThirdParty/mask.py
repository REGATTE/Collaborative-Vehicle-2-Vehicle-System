...
class MapMaskGenerator:
    ...
    def location_to_pixel(self, loc: carla.Location) -> Coord:
        ...
        if (not isinstance(loc.x, (int, float)) or 
            not isinstance(loc.y, (int, float)) or
            np.isinf(loc.x) or np.isinf(loc.y) or
            np.isnan(loc.x) or np.isnan(loc.y)):
        ...
        try:
            ...
            raise ValueError("Coordinate conversion overflow")
    ...
    def pedestrians_mask(self, pedestrians: List[carla.Actor]) -> Mask:
        ...
        for ped in pedestrians:
            ...
            # Get pedestrian transform and validate corner locations
            transform = ped.get_transform()
            transform.transform(corners)
            
            # Validate corner coordinates before conversion
            valid_corners = True
            for corner in corners:
                if (not isinstance(corner.x, (int, float)) or 
                    not isinstance(corner.y, (int, float)) or
                    np.isinf(corner.x) or np.isinf(corner.y) or
                    np.isnan(corner.x) or np.isnan(corner.y)):
                    valid_corners = False
                    break
            ...
            try:
                pixel_corners = [self.location_to_pixel(loc) for loc in corners]
                cv.fillPoly(img=canvas, pts=np.int32([pixel_corners]), color=COLOR_ON)
            except (OverflowError, ValueError):
                continue
